...
var versionTxt:FlxText;
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("timebar.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("score.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

			versionTxt = new FlxText(5, FlxG.height - 24, 0, SONG.song.toUpperCase + " - " + CoolUtil.difficultyString() + " | BOT | Nic Engine 0.5" , 40); 
			versionTxt.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
versionTxt.cameras = [camHUD];
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Song Score: ' + songScore + ' | Song Misses: ' + songMisses + ' | Song Accuracy: ' + ratingName;
		} else {
			scoreTxt.text = 'Song Score: ' + songScore + ' | Song Misses: ' + songMisses + ' | Song Accuracy: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
